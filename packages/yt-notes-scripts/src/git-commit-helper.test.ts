...
describe("commitFile", () => {
  test("committet eine neue Datei", async () => {
  ...
  test("no-op wenn Datei nicht dirty", async () => {
    const result = await commitFile(repo, "youtube/foo.md", "test commit")
    expect(result.committed).toBe(false)
    expect(result.reason).toBe("no changes")
  })

  test("kein throw wenn vaultRoot kein git-Repo ist", async () => {
  ...
  test("committet nur die spezifizierte Datei (nicht andere dirty Files)", async () => {
...
describe("commitAllInVault", () => {
  test("committet alle Files unter pathSpec, lässt andere dirty Files in Ruhe", async () => {
    mkdirSync(join(repo, "youtube"), { recursive: true })
  ...
  test("no-op wenn nichts unter pathSpec dirty ist (aber andere Files dirty sind)", async () => {
    mkdirSync(join(repo, "sonstwas"), { recursive: true })
    writeFileSync(join(repo, "sonstwas/bar.md"), "bar")
  ...
  test("no-op wenn Vault komplett clean", async () => {
    const result = await commitAllInVault(repo, "cleanup youtube", "youtube/**")
    expect(result.committed).toBe(false)
    expect(result.reason).toBe("no changes")
  })
