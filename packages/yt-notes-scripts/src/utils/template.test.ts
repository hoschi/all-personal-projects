...
test("processTemplate simple key replacement", () => {
  expect(processTemplate("Hello {{name}}!", { name: "world" }, SETTINGS)).toBe(
    "Hello world!",
  )
})

test("processTemplate unknown key → empty string", () => {
  expect(processTemplate("X={{missing}}", {}, SETTINGS)).toBe("X=")
})

test("processTemplate multiple replacements", () => {
  expect(
    processTemplate("{{a}} and {{b}}", { a: "one", b: "two" }, SETTINGS),
  ).toBe("one and two")
})

test("processTemplate chapters as string[] → format applied per item", () => {
  const data = { chapters: ["0:00 Intro", "1:00 Body"] }
  const out = processTemplate("Chapters:\n{{chapters}}", data, SETTINGS)
  expect(out).toBe('Chapters:\n - "0:00 Intro"\n - "1:00 Body"\n')
})

test("processTemplate hashtags as string[] → format applied per item", () => {
  const data = { hashtags: ["typescript", "bun"] }
  const out = processTemplate("Tags:\n{{hashtags}}", data, SETTINGS)
  expect(out).toBe('Tags:\n - "#typescript"\n - "#bun"\n')
})

test("processTemplate empty chapter array → empty string", () => {
  expect(processTemplate("X:{{chapters}}", { chapters: [] }, SETTINGS)).toBe(
    "X:",
  )
})

test("processTemplate empty data object → all keys become empty", () => {
