...
| -------------------------------------- | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1 audit (Werbung + Sektionierung)      | `sonnet` | `medium` | Sektionierung mit gegebenen Chapters ist deterministisch (alle Modelle identisch). Werbung-Erkennung braucht Kontext, aber Pass-1-Regel "bei Unsicherheit drin lassen" macht Sonnet sicher.                                    |
| 2 asr-fix (ASR-Fehler korrigieren)     | `sonnet` | `medium` | Domain-Verständnis nötig. Sonnet ~80% der Fixes (vergleichbar zu Opus, andere blinde Flecken). Haiku **disqualifiziert** — versteht Domain-Vokabular nicht (Effect-Begriffe wie `Redacted`, `PgLive`, `@effect/sql` verpasst). |
...
- `low`: Mini-Outputs ohne Recherche-Bedarf (Pass 3, 4).
- `medium`: klare Aufgaben mit Domain-Verständnis (Pass 1, 2).
