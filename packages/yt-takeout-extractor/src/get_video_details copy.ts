import { Effect, pipe, Data, Schema, Context, Layer, Command, Args, CliCommand, Console, Duration, FileSystem, String, Schedule } from "effect";
            ...
            getYoutubeApiKey: () =>
                apiKey
                    ...
                    : Effect.fail(new ValidationError({
                        ...
                        message: "YouTube API Key ist nicht gesetzt. Bitte .env Datei prüfen."
                    }))
        };
    })
);
...
const DatabaseServiceLive = Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        return {
            saveVideoDetails: (video: VideoData) => Effect.tryPromise({
                try: () => client.query(
            ...
            videoExists: (videoId: string) => Effect.tryPromise({
                try: () => client.query(
...
const extractChaptersFromDescription = (description: string): Effect.Effect<Chapter[], ValidationError> =>
    Effect.gen(function* () {
...
const getVideoData = (videoId: string): Effect.Effect<VideoData, YouTubeApiError | ValidationError> =>
    Effect.gen(function* () {
        // Validate video ID
        ...
        const response = yield* Effect.tryPromise({
            try: () => axios.default.get<YouTubeApiResponse>(`${YOUTUBE_API_BASE_URL}/videos`, {
                params: {
            ...
            catch: (error) => new YouTubeApiError({
                videoId,
        ...
        const validatedResponse = yield* Schema.parse(YouTubeApiResponseSchema)(response.data).pipe(
            Effect.mapError((e) => new YouTubeApiError({
                videoId,
        ...
        if (validatedResponse.items.length === 0) {
            yield* Effect.fail(new YouTubeApiError({
                videoId,
...
const saveVideoDetails = (video: VideoData): Effect.Effect<void, DatabaseError> =>
    Effect.gen(function* () {
...
const processVideo = (videoId: string): Effect.Effect<void, YouTubeApiError | ValidationError | DatabaseError> =>
    Effect.gen(function* () {
