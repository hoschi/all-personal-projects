...
import { SnapshotDetails } from "./schemas"
import {
...
export async function getMatrixData(
  ...
  const details: SnapshotDetails[] = Option.isNone(snapshotsResult)
    ? []
    : Option.getOrThrow(snapshotsResult).slice().reverse()
  const isInitialState = details.length === 0
  const lastDate = details.at(-1)?.snapshot.date ?? null
