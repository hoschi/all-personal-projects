...
import { DATABASE_SCHEMA_NAME, serverEnv } from "./env"
...
export async function executeWithSchema<T>(
...
export async function createAccount(
  ...
  debug(
    "Creating account: %s, category: %s, balance: %d",
    name,
    category,
    currentBalance,
  )
...
export async function deleteAccount(id: string): Promise<boolean> {
  ...
  try {
    ...
    const balanceCount = await executeWithSchema(
      async (db) =>
 ...
 */
export async function getLatestAssetSnapshot(): Promise<
  Option.Option<AssetSnapshot>
> {
...
export async function createAssetSnapshot(
  ...
  debug(
    "Creating asset snapshot: date=%s, totalLiquidity=%d",
    date,
    totalLiquidity,
  )
  try {
...
export async function getSnapshotDetails(
  limit: number,
): Promise<Option.Option<SnapshotDetails[]>> {
  ...
  try {
    ...
    const snapshots = await executeWithSchema(
      async (db) =>
        await db<AssetSnapshot[]>`
      SELECT id, date, total_liquidity as "totalLiquidity"
      FROM asset_snapshots
    ...
    for (const snapshot of snapshots) {
      const balanceDetails = await executeWithSchema(
        async (db) =>
          await db<AccountBalanceDetail[]>`
...
// ============================================================================
// Account Balance Detail Operations
// ============================================================================

/**
 * Get balance details for a specific snapshot
 */
export async function getBalanceDetailsBySnapshotId(
  snapshotId: string,
...
export async function createBalanceDetailsForSnapshot(
  ...
  debug(
    "Creating balance details for snapshot: %s, accounts: %d",
    snapshotId,
 ...
 */
export async function getRecurringItemsByInterval(
...
export async function createRecurringItem(
  ...
  debug(
    "Creating recurring item: %s, amount: %d, interval: %s",
    name,
    amount,
    interval,
  )
 ...
 */
export async function updateForcastScenario(
  data: ForecastScenarioChanges,
): Promise<undefined> {
 ...
 */
export async function updateScenarioIsActive(
  id: string,
  isActive: boolean,
): Promise<ScenarioItem> {
